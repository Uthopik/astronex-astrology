...
# En Python 3 usamos importaciones relativas con el punto
try:
    from .utils import dectodeg, parsestrtime, strdate_to_date
    from .nexdate import NeXDate
except ImportError:
    from .utils import dectodeg, parsestrtime, strdate_to_date
    from .nexdate import NeXDate

# Intentamos importar drawing, si da error circular lo manejamos
try:
...
conj_class = [[(0,4),(0,6),(0,9),(3,4),(3,9),(4,6),(4,7),(4,8),(4,9),(6,7),(7,9)],
            ...
            [(0,2),(1,2),(1,4),(2,3),(2,4),(2,5),(2,8),(2,9),(3,7),(3,8),(5,6),(5,8),(7,8)]]
...
class Chart(object):
    ...
    def calc(self, date, loc, flag):
        jd = pysw.julday(*date)
        planets = []
        for i in range(11):
            target = i
            if i == 10: target = 11  # Nodo Lunar
            res, pos, error = pysw.calc(jd, target, flag)
            planets.append(pos[0])
        
        # El simulador de pysw que creamos espera (jd, lat, lon, hsys)
        houses, asmc = pysw.houses_ex(jd, loc.latdec, loc.longdec, 'P')
        return planets, list(houses)

    def get_plan_long(self):
        return self.planets

    def house_plan_long(self):
        # Convertimos map a list para Python 3
        return list(map(self.get_house_long, self.planets))

    def get_house_long(self, plan):
        h = self.houses
        for i in range(12):
            if i == 11:
                if h[11] < h[0]:
                    if plan >= h[11] or plan < h[0]: return self.pos_in_house(plan, h[11], h[0], 11)
                else:
                    if plan >= h[11] and plan < h[0]: return self.pos_in_house(plan, h[11], h[0], 11)
            else:
                if h[i] < h[i+1]:
                    if plan >= h[i] and plan < h[i+1]: return self.pos_in_house(plan, h[i], h[i+1], i)
                else:
                    if plan >= h[i] or plan < h[i+1]: return self.pos_in_house(plan, h[i], h[i+1], i)
        return 0

    def pos_in_house(self, plan, h1, h2, i):
        diff = h2 - h1
        if diff < 0: diff += 360
        plan_diff = plan - h1
        if plan_diff < 0: plan_diff += 360
        # Divisiones normales en Python 3 devuelven float, que es lo que queremos aquí
        return (i * 30) + (plan_diff / diff) * 30

    def get_aspects(self, orbs, discard=[]):
        self.asp = []
        pl = self.planets
            ...
            for j in range(i+1, len(pl)):
                ...
                for k, orb in enumerate(orbs):
                    ...
                    if abs(diff - asp_deg) < orb:
                        self.asp.append({"p1": i, "p2": j, "asp": k, "orb": diff - asp_deg})
        return self.asp

    def chiron_calc(self, date, flag):
        jd = pysw.julday(*date)
        res, pos, error = pysw.calc(jd, 15, flag)
        return pos[0]

    def vulcan_calc(self, date, flag):
        # El cálculo de Vulcano suele ser relativo al sol si no está en efemérides
        jd = pysw.julday(*date)
        res, pos, error = pysw.calc(jd, 0, flag)
        return (pos[0] + 3.0) % 360
    ...
    def aspects(self, kind='radix'):
        # 1. Definimos las tablas NECESARIAS dentro de la función
        huber_matrix = [
        ...
        p_classes = [0, 1, 2, 2, 0, 1, 1, 2, 2, 2, 0]
        ...
        for i in range(len(pl)):
            for j in range(i + 1, len(pl)):
                # --- LÓGICA DE GRUPOS ---
                pci = p_classes[i] if i < len(p_classes) else 3
                pcj = p_classes[j] if j < len(p_classes) else 3
                
                # Regla del Nodo (10 hereda del planeta)
                if j == 10: pcj = pci
                if i == 10: pci = pcj
                ...
                # --- ACCESO A LA MATRIZ (Ahora sí está definida) ---
                orb1 = huber_matrix[pci][acl]
                orb2 = huber_matrix[pcj][acl]
                orb_max = max(orb1, orb2)

                if orb <= orb_max:
                    f1 = orb / max(0.1, orb1)
                    f2 = orb / max(0.1, orb2)
                    chart_orbs.append({
                        "p1": i, "p2": j, "a": nsig_idx, 
                        "f1": f1, "f2": f2, 'gw': False 
                    })
                elif orb <= orb_max * 1.1:
                    chart_orbs.append({
                        "p1": i, "p2": j, "a": nsig_idx, 
                        "f1": 0, "f2": 0, 'gw': True 
                    })
    ...
    def sizes(self):
        hs = self.houses
        # SEGURIDAD: Si no hay casas, devolvemos 30 grados por casa (zodiaco regular)
        if not hs or len(hs) < 7:
            return [30.0] * 12
...
######### cross points
    def calc_cross_points(self,cross=None):
        sizes = self.sizes()
        if not hasattr(self, 'houses') or not self.houses or len(self.houses) < 12:
            # Devolvemos 4 valores (ASC, MC, DESC, IC) en 0.0
...
######## dynchart
    ...
    def plan_conflicts(self):
        ...
        pl = sorted(pl, key=lambda x: list(x.values())[0])
        for i in range(len(pl)):
...
# --- Valores necesarios para el motor de dibujo ---
planclass = [0,1,2,2,0,1,1,2,2,2,0]
aspclass = [0,2,1,0,1,2,0,2,1,0,1,2]
orbs = [8,1,3,6,6,1,8,1,6,6,3,1]
