...
class OpPanel(Gtk.Box):
    ...
    def append_mult_group(self, name, pos, page, offset=3):
        ...
        # Usamos new_from_widget que es más estable en GTK 3
        button = Gtk.RadioButton.new_from_widget(self.sections_button_group)
        
        img = Gtk.Image()
        import os
        imgfile = os.path.join(appath, "astronex/resources", self.images[name])
    ...
    def append_single_group(self, name, group):
        ...
        # Simplificamos la creación del grupo
        button = Gtk.RadioButton.new_from_widget(self.sections_button_group)
        ...
        img = Gtk.Image()
        # Asegúrate de que path.joinpath funcione o usa os.path.join
        import os
        imgfile = os.path.join(appath, "astronex/resources", self.images[name]) 
        img.set_from_file(imgfile)
        
        button.set_image(img)
        button.set_mode(False) # Esto hace que parezca un botón normal, no un radio
        button.page = page
        button.name = name
        button.connect('toggled', self.on_catalog_button_toggled)

        pos = self.namebuttons.index(name) 
        r = pos % 2 ; c = int(pos / 2) # Forzamos a entero para la tabla
        self.groups_table.attach(button, c, c+1, r, r+1)
        ...
        try:
           ...
           if curr.opmode == 'double':
                ...
                if curr.clickmode == 'click':
                    ...
                    if i is None or tuple(m.get_path(i)) != (2,): #nodal
