...
class DrawPlagram(Gtk.DrawingArea):
    ...
    def on_check_toggled(self, menuitem):
        # En GTK3 usamos get_label() directamente en el menuitem
    ...
    def on_da_clicked(self, da, event):
        info = getattr(da, "move_info", None)
        if not info:
            info = {}
            info['button'] = -1
            da.move_info = info

        # En Python, para el doble clic se usa el número sin guion bajo
        if event.type == Gdk.EventType._2BUTTON_PRESS and event.button == 1:
            ...
            if event.button == 3:
                # popup_at_pointer es la forma moderna y recomendada en GTK3
            ...
            # En GTK3 get_pointer ya no existe. Se usa el "DeviceManager"
            # o más fácil aún: los atributos x e y del evento de movimiento
            x = event.x
            y = event.y
            
            # Si necesitas las coordenadas relativas a la ventana:
            # En GTK3 'allocation' es un método
    ...
    def dispatch(self, da, cr):
        # 1. En GTK3, 'cr' ya viene como argumento. No usamos cairo_create().
        # 2. Obtenemos el ancho y alto mediante el método get_allocation()
        alloc = da.get_allocation()
        w = alloc.width
        h = alloc.height
        ...
        if self.reset: 
            self.reset = False
            # Opcional: podrías resetear self.zoom = 1.0 aquí si quieres
        else:
            if self.do_zoom:
                z = self.zoom
                # Aplicamos el zoom
                ...
                # Cálculo de la posición del zoom
                ux, uy = cr.user_to_device(self.zx, self.zy)
                cr.translate(self.zx - ux, self.zy - uy)
        
        if self.panning:
            # Aplicamos el desplazamiento (panning)
            # El asterisco (*) expande la tupla que devuelve device_to_user_distance
            cr.translate(*cr.device_to_user_distance(self.pan_x, self.pan_y))
            
        cr.set_line_join(cairo.LINE_JOIN_ROUND) 
        cr.set_line_width(float(self.opts.base)) 
        
        # Llamamos al dibujante real pasándole el contexto de Cairo 'cr'
        self.drawer.dispatch_simple(cr, w, h) 
        
        return False
        
    def redraw(self): 
        # 1. Usamos get_allocation() en lugar del atributo directo
        alloc = self.get_allocation()
        w = alloc.width
        h = alloc.height
        
        try:
            # 2. En GTK3, la forma correcta de forzar el redibujado
            # de un área es queue_draw_area o simplemente queue_draw()
            self.queue_draw_area(0, 0, w, h)
        except Exception:
            # Usamos una excepción más genérica por seguridad
            pass
    ...
    def pdf_export(self):
        # Todo este bloque DEBE tener 4 espacios más que el 'def'
        ...
        # Este es el return que causaba el error
        if not filename: 
            return

        # --- Lógica de Cairo para PDF ---
        w = PDFH
        h = PDFW
