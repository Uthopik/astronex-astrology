...
class Slot(Gtk.VBox):
    ...
    def on_storage_clicked(self,but):
        ...
        self.storage_but.modify_bg(Gtk.StateType.NORMAL,Gdk.color_parse("white"))
        other.storage_but.modify_bg(Gtk.StateType.NORMAL,Gdk.color_parse("#f6f7fe"))
    ...
    def on_slot_clicked(self,slot,event):
        ...
        if event.type == Gdk.EventType.BUTTON_PRESS and event.button == 1:
            if self.wname != MainPanel.active_slot:
                other = MainPanel.pool[self.other]
                self.eb.modify_bg(Gtk.StateType.NORMAL,Gdk.color_parse("white"))
                other.eb.modify_bg(Gtk.StateType.NORMAL,Gdk.color_parse("#f6f7fe"))
                self.eye.show()
                other.eye.hide() 
...
class ChartBrowser(Gtk.VBox):
    def __init__(self, ap_path, font):
        ...
        # 1. Definimos primero la ruta base usando el ap_path que recibe la función
        # Usamos os.path.join para evitar errores de tipos de objeto
        self.appath = os.path.join(ap_path, 'astronex')
        
        # 2. Ahora sí definimos la ruta de recursos
        self.res_path = os.path.join(self.appath, 'resources')
        ...
        self.tables.pack_start(cell, True)
        ...
        for i, r in enumerate(liststore):
            # Usamos getattr para que si no existe 'db', no se rompa el programa
            db_actual = getattr(curr, 'db', getattr(curr, 'database', None))
            
            if db_actual and r[0] == db_actual:
                index = i
                break
        self.tables.set_active(index)
        
        hbox = Gtk.HBox()
        hbox.pack_start(self.tables, False, False, 0)
        
        opbut = Gtk.Button()
        img = Gtk.Image()
        imgfile = os.path.join(self.appath, "resources", "folder-convert24.png")
        ...
        opbut = Gtk.Button()
        img = Gtk.Image()
        imgfile = os.path.join(self.appath, "resources", "pgram.png")
        img.set_from_file(imgfile)
        opbut.set_image(img) 
        ...
        # 1. Intentamos abrir la base de datos físicamente
        import sqlite3
        import os
        
        db_path = os.path.join("resources", "charts.db")
        
        try:
            conn = sqlite3.connect(db_path)
            cursor = conn.cursor()
            # 2. Le preguntamos a la base de datos: "¿Qué tablas tienes?"
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
            tablas = cursor.fetchall()
            print(f"--- DIAGNÓSTICO: Las tablas en {db_path} son: {tablas} ---")
            conn.close()
        except Exception as e:
            print(f"--- ERROR DE DIAGNÓSTICO: {e} ---")

        # 3. Por ahora, para que no explote, pon un valor vacío
        chartlist = []
    ...
    def relist(self,new):
        ...
        for c in tablelist:
            liststore.append([c])
        if not new:
            model = self.tables.get_model()
        active_iter = self.tables.get_active_iter()

        if active_iter is not None:
            # Extraemos el texto de la primera columna (índice 0)
            new = model[active_iter][0]
        else:
            new = ''  # O el valor por defecto que prefieras si no hay selección
        index = 0
        for i,r in enumerate(liststore):
            if r[0] == new:
                index = i
    ...
    def on_tables_changed(self,combo): 
        ...
        if not self.chartview is None:
            ...
            idx = self.tables.get_active()
            if idx < 0: return # No hay nada seleccionado
        
            # Obtenemos el modelo y el texto de la fila activa
            model = self.tables.get_model()
            table_name = model[idx][0]
        
            # Ahora cargamos la lista de esa carpeta
    ...
    def on_chart_activated(self,view,path,col):
        ...
        id = model.get_value(iter,1)
        try:
            # Primero verificamos cuántas columnas tiene realmente el modelo
            n_cols = model.get_n_columns()

            # Si tiene 3 o más columnas, la 2 es segura. 
            # Si no, usamos el nombre de la tabla activa del selector de carpetas.
            if n_cols > 2:
                table = model.get_value(iter, 2)
            else:
                # Usamos la tabla que esté seleccionada en el combo de carpetas
                idx = self.tables.get_active()
                table = self.tables.get_model()[idx][0]
        except ValueError:
            table = self.tables.get_active_text()
    ...
    def new_chart(self, chart):
        ...
        # 1. Intentamos obtener el texto de la tabla
        if self.tables.get_has_entry():
            table = self.tables.get_child().get_text()
        else:
            table = self.tables.get_active_id()

        # 2. LIMPIEZA CRÍTICA: Si el texto es "(Buscar)" o está vacío, 
        # le asignamos una tabla por defecto (ej: "Radix" o "MisCartas")
        if not table or table == "(Buscar)":
            # Puedes poner aquí el nombre de tu tabla principal
            table = "Radix" 
        ...
        try:
            ...
            # Aquí ya enviamos un nombre de tabla seguro ("Radix" en lugar de "(Buscar)")
