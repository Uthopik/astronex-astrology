...
class CoreMixin(object):
    ...
    def d_year_lines(self,cr,radius,chartob):
        chart = chartob.chart
        try:
            year_str = chart.date.split('-')[0]
            year = int(year_str) - 1 if year_str else 2023
        except (ValueError, IndexError, AttributeError):
            year = 2023 # Año por defecto si falla
    ...
    def d_radial_lines(self,cr,radius,chartob):
        sign_cusps = iter(chartob.get_sign_cusps())
        off = next(sign_cusps)
    ...
    def d_cross_points(self,cr,radius,chartob):
        ...
        offset = chartob.get_cross_offset()
        for i in (0,180):
            angle = chartob.sup_cross(offset - cross - i) * RAD#Error 
    ...
    def draw_urnodal_planets(self,cr,radius,chartob,plot="plot1"):
        cusps = iter(chartob.get_cusps_offsets())
        off = next(cusps)
    ...
    def make_plines(self,cr,radius,chartob,pos='EXT',plot="plot1"):
        ...
        for glyph, plt in zip(glyphs, plots):
            # SEGURIDAD: Si el planeta (plt) no existe, saltamos al siguiente
            if plt is None:
                continue
    ...
    def d_urnod_radial_lines(self,cr,radius,chartob):
        sign_cusps = iter(chartob.chart.nod_sign_long())
        off = next(sign_cusps)
        fac,ins,_ = chartob.get_radial_param()
        radius = radius * fac
        cr.save()
    ...
    def make_spec_ruler(self,cr,radius,chartob,rule,rules,offset):
        ...
        for i in range(360):
            if i % 30 == 0: 
                cr.set_source_rgb(*cols[int((asc + i/30)%4)])
