...
class BioMixin(object):
    def __init__(self,zodiac):
        ...
        if chart is not None:
            ...
            # Si no hay carta, definimos valores por defecto para que no falle
            prev_chart = None, None, None
    ...
    def set_bio_from_date(self,bio,frac):
        global _bio, ruler
        _bio = bio 
        # Obtenemos el rectángulo de asignación (allocation) de forma compatible con GTK 3
        alloc = self.surface.get_allocation()
        
        # Ahora usamos alloc.width en lugar de self.surface.allocation.width
        ruler[0] = (frac * self.gridw + self.hoff) / alloc.width
