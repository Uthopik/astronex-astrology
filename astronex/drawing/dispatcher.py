...
class DrawMixin(CoreMixin,ProgMixin,ProfileMixin,BioMixin,DiagramMixin,SheetMixin,PaarWabeMixin,PlanetogramMixin):
    ...
    def draw_transits(self,cr,width,height,chartob=None):
        ...
        # --- PRINT DIAGNÓSTICO RADIX ---
        plan1 = chartob.get_planets()
        #print("DEBUG PLAN 1 (Radix):", plan1)
        
        chartob.swap_charts()
        chartob.prepare_params2()
        self.draw_planets(cr,radius,chartob,plot='plot2')
        
        # --- PRINT DIAGNÓSTICO TRÁNSITOS ---
        plan2 = chartob.get_planets()
        #print("DEBUG PLAN 2 (Transit):", plan2)
        ...
        self.aspmanager.opts = self.opts
        
        # --- PRINT ANTES DE ASPECTOS ---
        #print("DEBUG: Llamando a manage_aspects con filter='transit'")
    ...
    def set_AP_DEG(ap=None):
        ...
        if ap:
            AP_DEG = ap
        elif hasattr(curr.now, 'houses') and len(curr.now.houses) > 0:
            AP_DEG = curr.now.houses[0]
        else:
            AP_DEG = 0 # Valor por defecto para que no falle el arranque
