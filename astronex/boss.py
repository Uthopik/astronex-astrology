...
class Manager(object):
    ...
    def set_mainwin(self,mainwin):
        ...
        # Ajuste para set_cursor: En GTK 3 a veces falla si no se especifica el path completo
        # Si da error después, lo revisaremos, por ahora corregimos el error actual:
        view.set_cursor(0) 
        view.grab_focus()

        # CAMBIO CRÍTICO: Usamos get_allocation() en lugar de .allocation
        allocation = self.da.get_allocation()
        self.da.drawer.hoff = allocation.width * 0.125
        self.da.drawer.gridw = self.da.drawer.hoff * 6
        try:
            self.da.drawer.set_AP_DEG()
        except (IndexError, AttributeError):
            print("DEBUG: Esperando a que se calculen las casas...")
